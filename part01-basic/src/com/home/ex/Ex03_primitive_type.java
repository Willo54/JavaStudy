...
public class Ex03_primitive_type {
	...
	public static void main(String[] args) {
		 ...
		 * 						10000000000L 이라고 표기를 해야 한다.(int 범위를 벗어나는 정수 값은 접미사 L 붙임)
		 * 		
		 * 		6) 만약 long 형으로 처리가 안되는 숫자는 BigInteger 클래스를 이용하면 된다.
		 * 
		 * 2. 실수형
		 * 		1) float
		 * 			(1) 크기 : 4바이트
		 ...
		 * 3. 문자형
		 * 		1) char
		 * 			(1) 크기 : 2바이트 (C언어 char는 1바이트)
		 * 			(2) 일반 문자 크기
		 * 					- 영문 : 1바이트
		 * 					- 한글 : 2바이트
		 * 			(3) 문자 표현 코드
		 * 					- ASCII(아스키) : 1바이트 영문 표준 코드(C)
		...
		// 주의사항
		System.out.println(ch1 + ch2); // 'A'의 코드값 + '한' 코드값 = 54685
		System.out.println("" + ch1 + ch2); // A한
		
		// 논리값
		// 논리값은 일반적으로 변수명이 is 로 시작한다.
		boolean isPostive; // 양수인가?
		isPostive = true;
		if(isPostive) {
